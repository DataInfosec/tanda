...
fun CheckinScreen(
    ...
    val localEvent = LocalCheckinEvent.current
    ...
    val enabled = remember { derivedStateOf {
        localEvent.stream.value is DesignStreamState.Success
                || localEvent.stream.value is DesignStreamState.Default
    } }
    val isLoading = remember { derivedStateOf {
        localEvent.stream.value is DesignStreamState.Loading
                || state.value is CheckinViewModel.State.Loading
    } }
    ...
    LaunchedEffect(Unit) { localEvent.viewModel(deviceId, 0) }
    LaunchedEffect(state.value) {
        val message = when (val current = state.value) {
            ...
            is CheckinViewModel.State.Error -> {
                ...
                "${err.message} - ${err.score} ${err.threshold}"
            }
        }
